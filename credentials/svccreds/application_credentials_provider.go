...
var (
	// ErrApplicationCredentialsNotFound is returned when the application
	// credentials can't be found in the process's environment.
	ErrApplicationCredentialsNotFound = errors.New("GOOGLE_APPLICATION_CREDENTIALS not found in environment")
)
...
func (p *ApplicationCredentialsProvider) filename() (string, error) {
	if p.Filename == "" {
		if p.Filename = os.Getenv("GOOGLE_APPLICATION_CREDENTIALS"); p.Filename == "" {
			return "", ErrApplicationCredentialsNotFound
		}
	}
